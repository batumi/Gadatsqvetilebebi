...
var extract = function($) {
  ...
  $('body').find('.language').each(function() {
    ...
    $(this).children('.chapter').each(function() {
      var chapter;
      try {
        chapter = $(this).children('.label')[0].innerHTML;
      } catch (err) {
        console.warn('This chapter did not download correctly.');
      }
      ...
      (function(chapterNum, chapterDiv, langCode) {
        var chapterNumber = 'chapter' + chapterNum;
        ...
        $(chapterDiv).find('.verse').each(function() {
          ...
          try {
            ...
            var verseNumber = 'verse' + verseNum;
  ...
  for (var chapter in chapters) {
    for (var verse in chapters[chapter].verses) {
      var metadata = book + ':' + chapter.replace("chapter", "") + ':' + chapters[chapter].verses[verse]._verseNumber;
      for (var language in chapters[chapter].verses[verse]) {
  ...
  finalJSON = '{"book" : {"_book":"' + book + '", "chapters":' + JSON.stringify(chapters) + '}}';
  if (X2JS) {
    X2JS = new X2JS();
    xmlDocStr = X2JS.json2xml_str(JSON.parse(finalJSON));
  }
